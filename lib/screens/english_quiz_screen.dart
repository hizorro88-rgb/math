...
class _EnglishQuizScreenState extends State<EnglishQuizScreen> {
  ...
  void _speakQuestion() =>
      Speech.speak(_question.speech, lang: _question.speechLang);
  ...
  _EnChoiceState _choiceState(String choice) {
    if (!_answered) return _EnChoiceState.idle;
    if (choice == _question.answer) return _EnChoiceState.correct;
    if (choice == _selectedChoice) return _EnChoiceState.wrong;
    return _EnChoiceState.disabled;
  }
...
enum _EnChoiceState { idle, correct, wrong, disabled }

/// 낱말/글자/그림 보기 버튼
class _EnChoiceButton extends StatelessWidget {
  const _EnChoiceButton({
    required this.value,
    required this.emoji,
    required this.state,
    required this.onTap,
  ...
  Widget build(BuildContext context) {
    ...
    Widget button = AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: state == _EnChoiceState.idle
            ? [
                BoxShadow(
                  color: Colors.grey.shade300,
